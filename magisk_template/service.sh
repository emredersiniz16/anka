...
# DÜZELTME v6: Birleşik loglama (zaman damgalı) + OOM (-17) + Watchdog
#              Loglar: /data/local/tmp/anka_os.log (birincil)
#                       /cache/anka_os_kovan.log  (ikincil, cache partition)
...
if [ "$(getprop sys.boot_completed)" != "1" ]; then
    echo "[ANKA $(date '+%H:%M:%S')] HATA: Boot tamamlanmadi" > "$LOGFILE"
    exit 0
fi
...
# 8. Zaman damgalı log yazma fonksiyonu
#    C kodunun stderr/stdout + Python ajanının print/exception çıktıları
#    buraya gelir. Her satırın başına [YYYY-MM-DD HH:MM:SS] ekler.
log_ts() {
...
# 9. Süreç başlatma (log yönlendirme + OOM ile)
start_anka() {
